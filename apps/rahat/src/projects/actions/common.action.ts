...
export const beneficiaryActions: ProjectActionFunc = {
  ...
  [MS_ACTIONS.BENEFICIARY.BULK_ADD_TO_PROJECT]: (
    uuid,
    payload,
    sendCommand,
  ) => {
  ...
  [MS_ACTIONS.BENEFICIARY.ASSGIN_GROUP_TO_PROJECT]: (
    uuid,
    payload,
    sendCommand,
  ) => {
  ...
  [MS_ACTIONS.ELPROJECT.GET_VENDOR_REFERRER]: (uuid, payload, sendCommand) =>
    sendCommand({ cmd: BeneficiaryJobs.VENDOR_REFERRAL, uuid }, payload, 50000),
};
...
export const settingActions: ProjectActionFunc = {
  ...
  [MS_ACTIONS.SETTINGS.GET]: (uuid, payload, sendCommand) =>
    sendCommand({ cmd: ProjectJobs.PROJECT_SETTINGS_GET, uuid }, payload),
};
...
export const beneficiaryGroupActions: ProjectActionFunc = {
  [MS_ACTIONS.BENEFICIARY_GROUP.BULK_ASSIGN]: (uuid, payload, sendCommand) =>
    sendCommand(
      { cmd: ProjectJobs.BENEFICIARY_GROUP.BULK_ASSIGN, uuid },
      payload,
    ),
  ...
  [MS_ACTIONS.BENEFICIARY_GROUP.LIST_BY_GROUP]: (uuid, payload, sendCommand) =>
    sendCommand(
      { cmd: ProjectJobs.BENEFICIARY_GROUP.LIST_BY_GROUP, uuid },
      payload,
    ),
};
