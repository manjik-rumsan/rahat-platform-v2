...
export class BeneficiaryStatService {
  ...
  async totalVendors(projectUuid?: string) {
    ...
    if (projectUuid) {
      const vendorId = await this.prisma.projectVendors
        ...
        .then((projectVen) => projectVen.map((data: any) => data?.vendorId));
  ...
  async getTotalVoucher() {
    ...
    const tokenContract = await createContractReader(
      RahatTokenAbi,
      rahatTokenAddress,
    );
  ...
  async saveAllStats(projectUuid?: string) {
    ...
    if (projectUuid) {
      ...
      const projectStats = await this.client
        .send({ cmd: 'rahat.jobs.reporting.list', uuid: projectUuid }, {})
